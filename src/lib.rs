...
pub struct Options {
    /// Resize image before performing other operations. Dimensions must be
    /// non-zero; passing a one or more zeros will cancel resampling. In
    /// other words, pass `[0, 0]` to disable resampling. This is the
    /// default behavior.
...
pub fn convert<I>(_img: I, _opt: Options, _palette: ()) -> () {
    todo!()
}
